from ..trvae.unsupervised import trVAETrainer, tcrVAETrainer
import torch
import sys
...
class tcrembTrainer(tcrVAETrainer):
    ...
    def __init__(
        ...
        self.alpha = alpha # Group Lasso regularization coefficient
        self.omega = omega 
    ...
    def check_prox_ops(self):
        ...
        use_main = self.model.gene_decoder.L0.expr_L.weight.requires_grad

        use_prox_ops['main_group_lasso'] = use_main and self.alpha is not None # 0.7
        ...
        use_ext = use_ext and self.model.gene_decoder.L0.ext_L.weight.requires_grad
        use_prox_ops['ext_unannot_l1'] = use_ext

        use_ext_m = self.model.n_ext_m_decoder > 0 and self.alpha_l1 is not None
        use_ext_m = use_ext_m and self.model.gene_decoder.L0.ext_L_m.weight.requires_grad
    ...
    def apply_prox_ops(self):
        if 'main_soft_mask' in self.prox_ops:
            self.prox_ops['main_soft_mask'](self.model.gene_decoder.L0.expr_L.weight.data)
        if 'main_group_lasso' in self.prox_ops:
            self.prox_ops['main_group_lasso'](self.model.gene_decoder.L0.expr_L.weight.data)
        if 'ext_unannot_l1' in self.prox_ops:
            self.prox_ops['ext_unannot_l1'](self.model.gene_decoder.L0.ext_L.weight.data)
        if 'ext_soft_mask' in self.prox_ops:
            self.prox_ops['ext_soft_mask'](self.model.gene_decoder.L0.ext_L_m.weight.data)
    ...
    def on_epoch_end(self):
        if self.print_stats:
            if self.use_prox_ops['main_group_lasso']:
                n_deact_terms = self.model.gene_decoder.n_inactive_terms()
            ...
            if self.use_prox_ops['main_soft_mask']:
                ...
                share_deact_genes = (self.model.gene_decoder.L0.expr_L.weight.data.abs()==0) & main_mask
            ...
            if self.use_prox_ops['ext_soft_mask']:
                ...
                share_deact_ext_genes = (self.model.gene_decoder.L0.ext_L_m.weight.data.abs()==0) & ext_mask
            ...
            if self.use_prox_ops['ext_unannot_l1']:
                active_genes = (self.model.gene_decoder.L0.ext_L.weight.data.abs().cpu().numpy()>0).sum(0)
                print('Active genes in unannotated extension terms:', active_genes)
                sparse_share = 1. - active_genes / self.model.input_gene_dim
    ...
    def loss(self, total_batch=None):
        ...
        gene_recon_loss, tcr_recon_loss, tcr_hsic_loss, kl_loss, ot_loss = self.model(**total_batch)

        if self.beta is not None and self.model.use_hsic:
            weighted_hsic = self.beta * tcr_hsic_loss
            self.iter_logs["hsic_loss"].append(tcr_hsic_loss.item())
        else:
            weighted_hsic = 0.

        loss = gene_recon_loss + tcr_recon_loss + self.calc_alpha_coeff()*kl_loss + weighted_hsic + self.ot_weight*ot_loss

        self.iter_logs["loss"].append(loss.item())
        self.iter_logs["unweighted_loss"].append((gene_recon_loss + tcr_recon_loss + kl_loss + tcr_hsic_loss + ot_loss).item())
        self.iter_logs["gene_recon_loss"].append(gene_recon_loss.item())
        self.iter_logs["tcr_recon_loss"].append(tcr_recon_loss.item())
        self.iter_logs["tcr_hsic_loss"].append(tcr_hsic_loss.item())
        self.iter_logs["kl_loss"].append(kl_loss.item())
        self.iter_logs["ot_loss"].append(ot_loss.item())
