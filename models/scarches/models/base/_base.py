...
class BaseMixin:
    ...
    def load(
        ...
        # due to the different tcremb dataset, don't check adata
...
class CVAELatentsMixin:
    def get_latent(
        ...
        subsampled_indices = indices.split(512) # split data into batches with 512 data each batch
    ...
    def get_tcr_latent(
        ...
        tcremb = self.adata.obsm['tcremb']
        batchemb = self.model.embedding(c.to(dtype=torch.int))
        ...
        for batch in subsampled_indices:
            ...
            if issparse(x_batch):
                x_batch = x_batch.toarray()
            x_batch = torch.tensor(x_batch, device=device).to(torch.float32)
            # get tcr data
            tcremb_batch = torch.tensor(tcremb[batch], device=device).to(torch.float32)
            batchemb_batch = batchemb[batch,:]
            # get tcr latent
            latent = self.model.get_tcr_latent(x_batch, tcremb_batch, batchemb_batch, mean, mean_var)
            latent = (latent,) if not isinstance(latent, tuple) else latent
            latents += [tuple(l.cpu().detach() for l in latent)]

        # result = tuple(np.array(torch.cat(tensors, dim=0)) for tensors in latents)
        # result = tuple(np.array(torch.cat(tensors[0],dim=0)) for tensors in latents)
        # result = tuple(np.array(torch.cat(l,dim=0)) for l in zip(*latents))
        # result = tuple(np.array(torch.cat(l)) for l in zip(*latents))

        result_list = []
        # Iterate over grouped tensors
        for tensor_group in zip(*latents):
            # Concatenate tensors in the group
            concatenated = torch.cat(tensor_group, dim=0) # one embedding
            # Convert to NumPy array
            numpy_array = concatenated.cpu().numpy()
            # Append to the result list
            result_list.append(numpy_array)
...
class CVAELatentsModelMixin:
    ...
    def get_tcr_latent(self, x, tcremb, c=None, mean=False, mean_var=False):
        """Map `x` in to the latent space. This function will feed data in encoder  and return  z for each sample in
           data.
           Parameters
           ----------
        ...
        z1_mean_gene, z1_log_var_gene = self.gene_encoder(x_)
        z2_mean_tcr, z2_log_var_tcr = self.tcr_encoder(tcremb)
        latent_gene = self.sampling(z1_mean_gene, z1_log_var_gene)
        latent_tcr = self.sampling(z2_mean_tcr, z2_log_var_tcr)

        if mean:
            return z1_mean_gene, z2_mean_tcr # gene
        elif mean_var:
            # return (z_mean, torch.exp(z_log_var) + 1e-4)
            pass
        return latent_gene, latent_tcr
