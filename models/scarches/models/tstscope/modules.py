...
class MaskedCondLayers(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        if self.n_cond == 0:
            ...
            # split gene module data and cond data
        ...
        if cond is not None:
            # add the output of model and condition
...
class MaskedLinearDecoder(nn.Module):
    def __init__(
        ...
        print("\tMasked linear layer in, cond, out: ", in_dim, n_cond, out_dim)
        if mask is not None:
    ...
    def forward(self, z, batch=None):
        if batch is not None: # not used
            batch = one_hot_encoder(batch, n_cls=self.n_cond)
            z_cat = torch.cat((z, batch), dim=-1)
            dec_latent = self.L0(z_cat)
        else:
            dec_latent = self.L0(z) # dim z: gene module + cond embedding

        recon_x = self.mean_decoder(dec_latent)

        return recon_x, dec_latent # batch+decoder, decoder only
...
class ExtEncoder(nn.Module):
        ...
        if len(layer_sizes) > 1:
            ...
            for i, (in_size, out_size) in enumerate(zip(layer_sizes[:-1], layer_sizes[1:])):
                if i == 0:
                    ...
                    # Linear projection layer, mask = None
    ...
    def forward(self, x, batch=None):
        if batch is not None: # not add batch in encoder
...
class Decoder(nn.Module):
    ...
    def forward(self, z, batch=None):
        ...
        if self.recon_loss == "mse" or self.recon_loss == "identity":
