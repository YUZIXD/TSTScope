...
from .tstscope import TSTScopeCore
# from .regularized import tstscopeTrainer
from ..base._utils import _validate_var_names
from ..base._base import BaseMixin, SurgeryMixin, CVAELatentsMixin

class TSTSCOPE(BaseMixin, SurgeryMixin, CVAELatentsMixin):
    ...
    def __init__(
        ...
        conditions: Optional[list] = None, # condition key ?
        ...
        self.adata = adata
        # gene expression data + tcra and tcrb embeddings
        ...
        # gene input and tcr input, combind tcr matrix and gene expression into adata
        self.input_gene_dim_ = adata.X.shape[1]
        self.input_tcr_dim_ = adata.obsm['tcremb'].shape[1]
        ...
        self.model = TSTScopeCore(
            self.input_gene_dim_,
            self.input_tcr_dim_,
            self.latent_dim_,
            mask,
            self.conditions_,
    ...
    def train(
        ...
        **kwargs
        ...
        # Trainer class
        self.trainer = tstscopeTrainer(
            self.model,
            self.adata,
            alpha=alpha, # alpha, weight-1
            omega=omega, # omega, weigt-2, potential weight for tcr reconstruct
            condition_key=self.condition_key_,
            **kwargs
        )
        self.trainer.train(n_epochs, lr, eps) # trainer to train
    ...
    def nonzero_terms(self):
        ...
        """
        return self.model.gene_decoder.nonzero_terms()

    def get_tcr_latent(
        self,
        x: Optional[np.ndarray] = None,
        c: Optional[np.ndarray] = None, # batch or condition
        ...
        """
        result = super().get_tcr_latent(x, c, mean, mean_var)

        if not only_active:
            return result
        else:
            active_idx = self.nonzero_terms()
            active_idx = active_idx # add 2 TCR embeddings
